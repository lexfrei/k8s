...
- **`charts/`** - Vendored upstream Helm charts when consuming them directly is not possible. Currently holds `charts/obico/` — a byte-for-byte copy of `charts/obico` from the TheSpaghettiDetective/obico-server `release` branch (vendored 2026-07-10 @ `2a405848`): the upstream chart is not published to any Helm registry, and the obico-server repo is too heavy for ArgoCD to clone (built frontend bundles in history make a full fetch exceed any sane git timeout). Re-vendor by copying the directory to pick up upstream chart changes; drop the vendor if upstream ever publishes the chart as an OCI artifact. History: `charts/authelia/` (0.11.3 + local `secret_name` schema fix) was retired on 2026-04-25 once upstream 0.11.4 landed an equivalent fix; Authelia consumes `oci://ghcr.io/authelia/chartrepo` directly.
