...
- **`charts/`** - Reserved for vendored upstream Helm charts when consuming them directly is not possible. Currently empty. History: `charts/obico/` (byte-for-byte copy from the obico-server `release` branch) was vendored 2026-07-10 because the upstream repo was too heavy for ArgoCD to clone and no published chart existed; retired 2026-07-11 once upstream started publishing the chart as a signed OCI artifact (`oci://ghcr.io/thespaghettidetective/charts/obico`, images baked per-commit). `charts/authelia/` (0.11.3 + local `secret_name` schema fix) was retired 2026-04-25 once upstream 0.11.4 landed an equivalent fix; Authelia consumes `oci://ghcr.io/authelia/chartrepo` directly.
